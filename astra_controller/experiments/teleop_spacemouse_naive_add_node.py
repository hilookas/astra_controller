...
def main(args=None):
    ...
    goal_pos = np.concatenate([Tsinitgoal[:3, 3], pr.euler_from_matrix(Tsinitgoal[:3, :3], 0, 1, 2, True)])
    ...
    while True:
        ...
        action[:3] *= 100.0 / 1000.0 * control_T # mm/s
        action[3:] *= 30 / 180 * math.pi * control_T # deg/s
        ...
        pub_goal_cb("right", pt.transform_from(pr.matrix_from_euler(goal_pos[3:], 0, 1, 2, True), goal_pos[:3]))
